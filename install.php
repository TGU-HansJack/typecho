...
function install_step_3_perform()
{
    ...
    try {
        ...
        // 添加action路由到路由表
        $routingTable = $installDb->fetchRow($installDb->select()->from('table.options')->where('name = ?', 'routingTable'));
        if (!empty($routingTable)) {
            $routingTable = unserialize($routingTable['value']);
            if (is_array($routingTable)) {
                $routingTable['users-token'] = [
                    'url' => '/action/users-token',
                    'widget' => 'Widget_Users_TokenAction',
                    'action' => 'action'
                ];
                
                $installDb->query(
                    $installDb->update('table.options')
