...
class CommentsApi {
    ...
    public function create() {
        // 权限检查
        $user = Auth::getUser();
        Permission::require('comment_create', $user);
        ...
        $cid = (int)($b['cid'] ?? 0);
        $text = trim($b['text'] ?? '');
        $author = trim($b['author'] ?? ($user['screenName'] ?? 'Guest'));
        $mail   = trim($b['mail'] ?? ($user['mail'] ?? ''));
        $url    = trim($b['url'] ?? '');
    ...
    public function update($params) {
        ...
        // 权限检查
        $user = Auth::getUser();
        Permission::require('comment_delete', $user, $exist);
